...
public class HibernateItemGroupDao extends AbstractHibernateDao implements ItemGroupDao {
	...
	private static final String Q_GET_ITEM_GROUP_DETAILS = "" + "SELECT ig.ID " + ItemGroupDto.PROP_IG_ID + ",\n"
	...
	// generic thường dùng white card (?) khi chưa biết kiêu dữ liệu sử dụng là gì
	@SuppressWarnings({  "deprecation" })
	@Override
	public List<ItemGroupDto> getItemGroupDetails(Set<Integer> igIds) {
		NativeQuery<?> nativeQuery = openSession().createNativeQuery(Q_GET_ITEM_GROUP_DETAILS);
		nativeQuery.setParameter("pIgIds", igIds).addScalar(ItemGroupDto.PROP_IG_ID, IntegerType.INSTANCE)

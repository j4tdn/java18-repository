package view.stream.transaction;
...
public class demo {
	public static void main(String[] args) {
		...
		List<Transaction> transByYear = transactions.stream().filter(tran -> tran.getYear() == 2011)
		...
		List<Transaction> transByValue = transactions.stream().filter(tran -> tran.getValue() > 300)
				.sorted(Comparator.comparing(tran -> tran.getTrader().getCity())).collect(Collectors.toList());
		...
		List<Trader> traderByCity = traders.stream().filter(trader -> "Cambridge".equals(trader.getCity()))
		...
		String traderNames = traders.stream().map(Trader::getName).sorted().collect(Collectors.joining(", "));
		System.out.println("\n5. Return a string of all traders’ names sorted alphabetically");
		System.out.println("-------------------------------");
		System.out.println(traderNames);
		System.out.println("------------------//////////////-----------------\n");

		boolean isMilan = traders.stream().anyMatch(trader -> "Milan".equals(trader.getCity()));
		System.out.println("\n6. Are any traders based in Milan?");
		System.out.println("-------------------------------");
		System.out.println(isMilan);
		System.out.println("------------------//////////////-----------------\n");

		long tradersInMilan = traders.stream().filter(trader -> "Milan".equals(trader.getCity())).count();
		System.out.println("\n7. Count the number of traders in Milan");
		System.out.println("-------------------------------");
		...
		List<Transaction> transInCity = transactions.stream()
				.filter(tran -> "Cambridge".equals(tran.getTrader().getCity())).collect(Collectors.toList());
		print("8. Print all transactions’ values from the traders living in Cambridge", transInCity);

	    long highestValueOfTrans = transactions.stream().map(Transaction::getValue).reduce(Long.MIN_VALUE, Math::max);
		System.out.println("\n9. What’s the highest value of all the transactions?");
		System.out.println("-------------------------------");
		...
		Optional<Transaction> tranWithSmallestValue = transactions.stream()
				.min(Comparator.comparingLong(Transaction::getValue));
		System.out.println("\n10. Find the transaction with the smallest value");
		System.out.println("-------------------------------");
